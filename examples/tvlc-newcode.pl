...
while (<>) {
  ...
  #$stream->put_blocktaboo($p, $v);
  #$stream->put_fibgen($p, $v);
  $stream->put_comma($p, $v);
  #$stream->put_goldbach_g2($v);
  ...
  #my $d = $stream->get_blocktaboo($p);
  #my $d = $stream->get_fibgen($p);
  my $d = $stream->get_comma($p);
  #my $d = $stream->get_goldbach_g2();
  if ($d != $v) {
